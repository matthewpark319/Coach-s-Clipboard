                ...
                @if (count($schedule) > 0)
                    <ul class="list-group">
                        @foreach($team->scheduleComplete() as $s) 
                            <a href="{{ route('coach-view-meet-xc', ['meet' => $s->id]) }}" class="list-group-item">{{ $s->name }}</a>
                        @endforeach
                    </ul>
                @else 
...
function showTeamBests() {
    if ($('#event').val()) {
        if ($('#course').val()) window.location.href = "/coach/results-xc/team-bests/" + $("#event option:selected").val() + "/" + $("#course option:selected").val();
        else window.location.href = "/coach/results-xc/team-bests/" + $("#event option:selected").val();
    }
}
</script>
@endsection